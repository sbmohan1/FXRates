function createTable(){
	var table = "<tr>";
	var headers = ["","","USD","EUR","GBP","INR","AUD","CAD","ZAR","NZD","JPY"];
	var currencies = ["1 USD","1 EUR","1 GBP","1 INR","1 AUD","1 CAD","1 ZAR","1 NZD","1 JPY"]
	var exRate = [.2038, .9272, .62762, .86876, .7268, 1.27736, .0289783, .37863, .7573];
	...
	for(var i=0; i<currencies.length; i++){
		table += "<tr id= 'row"+i+"'><th>"+flags[i+2]+"</th><th>"+currencies[i]+"<br>"+"<span style='color: grey; font-size: 14px'>Inverse: </th>";
		for(var y=0; y<exRate.length; y++){
			table += "<td>"+exRate[y].toPrecision(5)+"<br>"+exRate[y].toPrecision(5)+"</td>";
		}
		table += "</tr>";
	}
